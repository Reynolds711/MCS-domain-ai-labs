...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.code-block').forEach(block => {
        ...
        copyButton.style.cssText = 'position: absolute; top: 10px; right: 10px; padding: 5px 10px; background: #1e3a5f; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.8rem;';
...
// Faculty pitch helper - tracks use case ideas
class FacultyPitchTracker {
    constructor() {
        this.storageKey = 'faculty_pitch_ideas';
    }
    ...
    exportIdeas() {
        const ideas = this.getIdeas();
        return ideas.map(i => `- ${i.professor}: ${i.useCase} (${i.estimatedROI})`).join('\n');
    }
}
